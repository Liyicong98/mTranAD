...
def load_data(dataset):
	...
	if dataset == 'synthetic':
		...
		for filename in file_list:
			...
			# vals:['135','UCR','Anomaly','InternalBleeding16','1200','4187','4199']
			...
			train, test = temp[:3000], temp[3000:]
		...
		for filename in file_list:
			...
			for timestamp in labeldict['realKnownCause/'+filename]:
				# print("打印一下timestamp", timestamp)
				tstamp = timestamp.replace('.000000', '')
				# print("打印一下timestamp.replace：",tstamp)
				index = np.where(((df['timestamp'] == tstamp).values + 0) == 1)[0][0]
				# print('打印一下index:',index)
				labels[index-4:index+4] = 1
				# print('打印一下labels', labels)
...
if __name__ == '__main__':
	...
	if len(commands) > 0:
		...
		# print("Usage: python preprocess.py SMAP MSL SWaT WADI SMD MSDS UCR MBA NAB")
