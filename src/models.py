...
class TranAD(nn.Module):
    ...
    def encode(self, src, c, tgt):
        ...
        tgt = tgt.repeat(1, 1, 2)
        # tgt是真实的elem
        # 需要把memory分成两半，均值和方差，高斯分布，重参数，去改transfomer_decoder的inputs的维度，tgt和memory的维度打印一下
...
class TranAD_VAE(nn.Module):
	def __init__(self, feats):
		super(TranAD_VAE, self).__init__()
		self.name = 'TranAD_VAE'
		self.lr = lr
		# self.batch = 128原来的 32还阔以
		self.batch = 16
		self.n_feats = feats
		self.n_window = 10
		self.n = self.n_feats * self.n_window
	...
	def encode(self, src, c, tgt):
		print('tranAD_VAE')
		# print("src.shape,c.shape,self.n_feats",src.shape,c.shape,self.n_feats)
		src = torch.cat((src, c), dim=2)
		...
		# tgt = tgt.repeat(1, 1, 2)
		# 拆分memory
		mu, sigma = memory[:,:,:self.n_feats],memory[:,:,self.n_feats:]
		# print("mu:",mu.shape,tgt.shape)
		# exit()
		return mu,sigma,tgt
		# return tgt,memory
	# 随机生成隐含向量z
	...
	def forward(self, src, tgt):
		# Phase 1 - Without anomaly scores
		c = torch.zeros_like(src)
		mu,std,tgt = self.encode(src,c,tgt)
		z = self.reparameterize(mu,std)
		# print("z.shape",z.shape)
		x1 = self.fcn(self.transformer_decoder1(z,tgt))

		# Phase 2 - With anomaly scores
		c = (x1 - src) ** 2
		mu, std,tgt = self.encode(src, c, tgt)
		z = self.reparameterize(mu, std)
		x2 = self.fcn(self.transformer_decoder2(z,tgt))
		return x1, x2, mu, std
